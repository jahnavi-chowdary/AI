...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            # Getting distance using Manhattan Distance
            truedist = util.manhattanDistance(p, pacmanPosition)

            if noisyDistance == None:
                continue
            elif emissionModel[truedist] > 0:
                prob = self.beliefs[p]*emissionModel[truedist]
                allPossible[p] = prob
				
        # Check if a ghost has been captured by the Pacman
        if noisyDistance == None:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        # For the 3rd Test Case we notice differences emerging in the shading of the squares.
        # The darker squares are the squares which are more probable Ghost Positions as compared to the squares with lighter colored sqaures.
        allPossible = util.Counter()
        for ghostpos_old in self.legalPositions:
			# Getting the Position to move to
            position_to_move = self.getPositionDistribution(self.setGhostPosition(gameState, ghostpos_old))
            ghostpos_new = position_to_move
            for key, val in ghostpos_new.items():
                newval = val * self.beliefs[ghostpos_old]
                allPossible[key] += newval
        self.beliefs = allPossible
